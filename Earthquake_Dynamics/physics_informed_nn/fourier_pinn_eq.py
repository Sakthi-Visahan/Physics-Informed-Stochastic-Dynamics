...
# 100-Sample Challenge
np.random.shuffle(raw_peaks)
x_max_samples = torch.tensor(raw_peaks[:100], dtype=torch.float32, device=device).unsqueeze(1)
print(f"Evaluating final plot on exactly {len(x_max_samples)} earthquake samples!")
...
# ==========================================
# 4. TWO-STAGE TRAINING LOOP (RESTORED TO OPTIMAL)
# ==========================================
epochs_adam = 45000
batch_size = 6000

model = FF_PDEM_PINN(mapping_size=128).to(device)
optimizer_adam = torch.optim.Adam(model.parameters(), lr=2e-3, weight_decay=1e-5)
...
for epoch in range(epochs_adam):
    ...
    loss_i = ic_loss(model, y_ic, x_max_ic, sigma=SIGMA_IC_END)
...
# ==========================================
# 5. HONEST SINGLE-PASS EVALUATION (MLE)
# ==========================================
print("Precomputing Universal PINN Matrix...")

model.eval()
y_plot_tensor = torch.tensor(x_gt, dtype=torch.float32, device=device).unsqueeze(1)
tau_final = torch.ones_like(y_plot_tensor)

x_eval_points = np.linspace(0.001, 0.25, 1000)
pinn_matrix = []

with torch.no_grad():
    for x_m in x_eval_points:
        x_m_tensor = torch.full_like(y_plot_tensor, float(x_m))
        p_val = model(y_plot_tensor, tau_final, x_m_tensor).cpu().numpy().flatten()
        pinn_matrix.append(p_val)

pinn_matrix = np.array(pinn_matrix)

print("Integrating PINN solutions using the strictly locked 100-sample dataset...")
sample_np = x_max_samples.cpu().numpy().flatten()

# ---------------------------------------------------------
# THE FIX: Single-Pass Maximum Likelihood Estimation (MLE)
# No loops, no averaging, no MoM distortions.
# ---------------------------------------------------------
shape, loc, scale = lognorm.fit(sample_np, floc=0)

weights = lognorm.pdf(x_eval_points, shape, loc=loc, scale=scale)
weights = weights / np.sum(weights) 

p_raw = np.dot(weights, pinn_matrix)

area = np.trapezoid(p_raw, x_gt)
p_normalized = p_raw / area if area > 0 else p_raw

rmse = np.sqrt(np.mean((p_normalized - p_gt) ** 2))
print(f"\n--- Honest 100-Sample Evaluation RMSE: {rmse:.4f} ---")

# Plotting
plt.figure(figsize=(10, 6))
plt.plot(x_gt, p_gt, 'b-', linewidth=3, label='MATLAB 20,000 MCS (Ground Truth)')
plt.plot(x_gt, p_normalized, 'r--', linewidth=2.5, label='FF-PINN (Honest 100-Sample MLE)')

plt.title('Dialed-In FF-PINN vs Ground Truth (100-Sample Challenge)', fontsize=14)
