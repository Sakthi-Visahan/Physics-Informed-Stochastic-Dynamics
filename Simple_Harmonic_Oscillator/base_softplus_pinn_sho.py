...
#losses, no gpinn
def standard_phy_loss(model, x, t, theta, x0=0.1):
...
def bc_loss(model, t_bc, theta_bc):
    x_left = torch.full_like(t_bc, -0.11)
    x_right = torch.full_like(t_bc, 0.11)
...
#softplus_goldilocks
print(f"Starting Base PINN (Softplus+Goldilocks) Training on {device}...")
...
for epoch in range(epochs):
    ...
    #same issue as gpinn model, move tensors to device
    ...
    if epoch < 1000:
        ...
        loss_p = standard_phy_loss(model, x_col, t_col, theta_col)
    ...
    if epoch % 100 == 0:
        ...
        print(f"Phase ({phase}) | Ep {epoch:04d} | Base PINN Loss: {loss.item():.4f} | Time: {elapsed_time:.2f}s")
        start_time = time.time()

print("Training Complete.")

#plotting
print("Generating Base PINN vs FDM Comparison Graphs...")
...
with torch.no_grad():
    for idx, t_val in enumerate(t_vals):
        ...
        # 2. Base PINN Prediction (Computed and Plotted)
        t_tensor = torch.full_like(x_tensor, t_val)
        p_marginal = torch.zeros_like(x_tensor)
        
        num_thetas = 100
        theta_range = torch.linspace(1.25 * math.pi, 1.75 * math.pi, num_thetas, device=device)
        ...
        # Domain masking and Normalization
        ...
        ax.plot(x_plot, p_scaled, color='m', linestyle='--', linewidth=3.0, label='Base PINN (Softplus)')
